...
from QLeaning import QLearning
...
q_learning = QLearning(epsilon=EPSILON, gamma=GAMMA)
env = Environment()
...
for i in range(EPISODES):
    ...
    while True:
        current_action = q_learning.pick_action(current_state)
        reward, next_state = env.do_action(current_action)

        # Update Values
        q_learning.update(current_state, current_action, next_state, reward)
...
best_action_for_state = q_learning.get_best_actions()
...
fig2.write_image("plots/qlearning.png")
fig2.show()
#print("Result")
#print(sarsa._Q)
