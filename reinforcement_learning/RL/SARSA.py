...
class SARSA:
    ...
    def update(self, state: Position, action: Action, next_state: Position, next_action: Action, reward: float) -> None:
        ...
        if self._env.is_terminal_state(next_state):
            ...
            next_state_action_val = self._Q[self._state_indexes[next_state], next_action.value]
